...
extract_article_links_prompt = """
# Task: Article Link Extraction from Page Summary

## Persona:
You are an expert content analyst specializing in web content structure and information extraction. Your job is to analyze a summary of a web page and accurately identify which links point to actual article/content pages, as opposed to navigation, author, comment, or utility pages.

## Input:
You are given a summary of a articles listing web page, which include a list of links, navigation elements, author references, and article references.

## Core Instruction - CRITICAL ADHERENCE REQUIRED:
Your task is to extract ONLY the links that point to actual article or content pages (for example: blog posts, research articles, documentation, or external resources). 
- **DO NOT** include links that point to navigation pages, author/user profiles, comment sections, "hide", "discuss", or other utility/non-content pages.
- **DO NOT** include links that point to internal site navigation or meta pages (such as Hacker News item pages, user pages, or comment threads).
- **DO NOT** include links whose URL contains `/item?id=`, `/user?id=`, `/hide?`, `/comments`, `/from?site=`, or similar patterns typical of navigation or meta pages.
- **INCLUDE** only links that point directly to external articles, blog posts, research papers, or documentation.

## Output Format:
Return ONLY a list of URLs (as strings) that point to article/content pages, in valid JSON format (e.g., ["https://example.com/article1", "https://example.com/article2"]).

## Example:

### Input Summary:
26. | [Reasoning by Superposition: A Perspective on Chain of Continuous Thought](https://arxiv.org/abs/2505.12514)  
34 points by [danielmorozoff](https://news.ycombinator.com/user?id=danielmorozoff) | [hide](https://news.ycombinator.com/hide?id=44309345&goto=news) | [1 comment](https://news.ycombinator.com/item?id=44309345)

### Output:
["https://arxiv.org/abs/2505.12514"]

### Input Summary:
Link: https://news.ycombinator.com/item?id=44309345, Text: Reasoning by Superposition: A Perspective on Chain of Continuous Thought  
Link: https://news.ycombinator.com/item?id=44309891, Text: Show HN: Trieve CLI – Terminal-based LLM agent loop with search tool for PDFs

### Output:
[]

## Now process the following summary and return ONLY the list of article/content URLs:
{summary}
